...
tf.flags.DEFINE_string('output_dir', './htm_v3',
...
def R_Shuffling_G_Shifting_B_Stretching(image,size,part,leaf):
    images_normalized = normalize_image(image)

    hist_r, hist_g, hist_b = calculate_histogram(image)
    hist_r = tf.random.shuffle(hist_r)
    hist_g = histogram_Shifting(hist_g,size)
    hist_b = histogram_Stretching(part,leaf,200,300)
...
def R_Shuffling_G_Shifting_B_Stretching_function(images,size,part,leaf):
    for i in range(images.shape[0]):
        single_image_tensor = images[i]
        new_image_tensor = R_Shuffling_G_Shifting_B_Stretching(single_image_tensor,size,part,leaf)
        images = tf.tensor_scatter_nd_update(images, [[i]], [new_image_tensor])
    return images

#只是用边缘的几个颜色值
def histogram_Stretching(part, leaf, minval=200, maxval=300, total_sum=89401):
...
def histogram_Shifting(hist,range_size):
...
def build_histogram(image, iter_i):
    ...
    histogram_image = R_Shuffling_G_Shifting_B_Stretching_function(image, 110, 110, 55)
    histogram.append(histogram_image)

    histogram_image = R_Shuffling_G_Shifting_B_Stretching_function(image, 50, 50, 25)
    histogram.append(histogram_image)

    histogram_image = R_Shuffling_G_Shifting_B_Stretching_function(image, 24, 24, 12)
    histogram.append(histogram_image)
