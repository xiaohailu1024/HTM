...
tf.flags.DEFINE_string('output_dir', './outputs_mi_16',
...
def graph(x, y, i, x_max, x_min, grad):
    ...
    first_round = tf.cast(tf.equal(i, 0), tf.int64)         #这个张量用于区分第一轮迭代和后续迭代。
    y = first_round * pred + (1 - first_round) * y              #在第一轮迭代中，使用预测的类别索引pred作为新的标签y；在后续迭代中，保持原来的标签y不变。
    one_hot = tf.one_hot(y, num_classes)                        #将标签y转换为独热编码形式
    cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits_v3)         #计算独热编码的标签one_hot和模型输出的 logits（logits_v3）之间的 softmax 交叉熵损失。
    noise = tf.gradients(cross_entropy, x)[0]                                   #计算交叉熵损失对输入数据x的梯度，得到噪声张量noise
    noise = noise / tf.reduce_mean(tf.abs(noise), [1, 2, 3], keep_dims=True)            #对噪声张量进行归一化处理
    noise = momentum * grad + noise             #结合上一次的梯度信息（通过grad参数传入）和当前计算得到的噪声，使用动量参数进行更新，以累积梯度信息。
    x = x + alpha * tf.sign(noise)              #根据噪声的符号和步长alpha对输入数据x进行更新
    # x = x + alpha * tf.sign(noise) * tf.math.tanh(tf.abs(noise))
    # x = x + alpha * tf.sign(noise) * tf.math.sigmoid(tf.abs(noise))
    # x = x + alpha * tf.sign(noise) * tf.nn.relu(tf.abs(noise))
    x = tf.clip_by_value(x, x_min, x_max)       #将更新后的输入数据x裁剪在x_min和x_max范围内，确保数据在合理的范围内
    i = tf.add(i, 1)                        #增加迭代计数器i的值，表示进行了一次迭代。
